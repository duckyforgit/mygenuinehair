        ...
        { 
          ...
          this.resultsDiv.append(` 
             ${response.testimonials.length ? '<div class="row gx-3">' : '<p>No general information matches that search.</p>'}
               ${response.testimonials.map(item => `<div class="col-sm-12 col-md-6 cell-testimonial"><blockquote>${item.clientTestimonial}<cite>${item.clientFirstName}</cite></blockquote></div>`).join('')}
