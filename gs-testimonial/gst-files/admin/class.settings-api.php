 ...
 */
if ( !class_exists( 'GS_T_WeDevs_Settings_API' ) ):
class GS_T_WeDevs_Settings_API {
    ...
    function admin_enqueue_scripts( $hook ) {
        ...
        wp_register_script( 'gstRangesliderJs', GST_FILES_URI . '/admin/js/gst.rangeslider.min.js', array( 'jquery' ), GST_VERSION, false );
        wp_register_script( 'gstSwitchButtonJs', GST_FILES_URI . '/admin/js/gst.jquery.switchButton.js', array( 'jquery' ), GST_VERSION, false );
        ...
        wp_register_style( 'gstSwitchButtonStyle', GST_FILES_URI . '/admin/css/gst.jquery.switchButton.css', '', GST_VERSION );
        wp_register_style( 'gstRangesliderStyle', GST_FILES_URI . '/admin/css/gst.rangeslider.css', '', GST_VERSION );
    ...
    function callback_range( $args ) {
        ...
        $html = sprintf( '<output class="wpb_output"></output>
    ...
    function callback_switch( $args ) {
        ...
        $html = sprintf( '<input type="hidden" name="%1$s[%2$s]" value="off" />', $args['section'], $args['id'] );
        $html .= sprintf( '<input type="checkbox" class="wpb_switch" id="wpuf-%1$s[%2$s]" name="%1$s[%2$s]" value="on"%4$s %5$s/>', $args['section'], $args['id'], $value, checked( $value, 'on', false ),  $switch_default );
        $html .= sprintf( '<label class="gst_switch_label" for="wpuf-%1$s[%2$s]"> %3$s</label>', $args['section'], $args['id'], $args['desc'] );
