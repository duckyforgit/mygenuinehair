...
class Recent_Posts_Sidebar_Widget extends WP_Widget {
	...
	function __construct() {
		...
		parent::__construct(
			...
			esc_html__( 'Recent Posts Sidebar(FoundationPress)', 'foundationpress' ),  
			array(
				'classname' => 'recent-posts-sidebar',
				'description' => esc_html__( 'Displays recent posts in sidebar.', 'foundationpress' ),
	...
	public function delete_widget_cache() {
		...
		wp_cache_delete( 'recent_posts_sidebar', 'widget' );
	...
	function widget( $args, $instance ) {
		...
		if ( ! $this->is_preview() ) {
			$cache = wp_cache_get( 'recent_posts_sidebar', 'widget' );
		}
		if ( ! is_array( $cache ) ) {
			$cache = array();
		}
		...
		if ( ! $this->is_preview() ) {
			...
			wp_cache_set( 'recent_posts_sidebar', $cache, 'widget' );
		} else {
			ob_end_flush();
		}
	}
	...
	function postmeta( $settings ) {
		...
		if ( true == $settings['meta_author'] ) : ?>
			...
			<span class="meta-author meta-author">
				<?php printf('<a href="%1$s" title="%2$s" rel="author">%3$s</a>',
					esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ),
					esc_attr( sprintf( esc_html__( 'View all posts by %s', 'foundationpress' ), get_the_author() ) ),
					get_the_author()
				);?>
			</span>
		...
		if ( true == $settings['meta_comments'] and comments_open() ) : ?>
			...
			<span class="meta-comments">
				<?php comments_popup_link( esc_html__( 'No comments', 'foundationpress' ), esc_html__( 'One comment','foundationpress' ), esc_html__( '% comments','foundationpress' ) ); ?>
			</span>
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'category' ); ?>"><?php esc_html_e( 'Select Category:', 'foundationpress' ); ?></label><br/>
			<?php
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'order' ); ?>"><?php esc_html_e( 'Order by:', 'foundationpress' ); ?></label><br/>
			<select name="<?php echo $this->get_field_name( 'order' ); ?>" id="<?php echo $this->get_field_id( 'order' ); ?>">
				<option <?php selected( $settings['order'], 'date' ); ?> value="date"><?php esc_html_e( 'Post Date', 'foundationpress' ); ?></option>
				<option <?php selected( $settings['order'], 'comment_count' ); ?> value="comment_count"><?php esc_html_e( 'Comment Count', 'foundationpress' ); ?></option>
				<option <?php selected( $settings['order'], 'rand' ); ?> value="rand"><?php esc_html_e( 'Random', 'foundationpress' ); ?></option>
			</select>
		</p>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'thumbnails' ); ?>">
				...
				<?php esc_html_e( 'Display post thumbnails', 'foundationpress' ); ?>
			</label>
		</p>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'meta_date' ); ?>">
				...
				<?php esc_html_e( 'Display post date', 'foundationpress' ); ?>
			</label>
		</p>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'meta_author' ); ?>">
				...
				<?php esc_html_e( 'Display post author', 'foundationpress' ); ?>
			</label>
		</p>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'meta_comments' ); ?>">
				...
				<?php esc_html_e( 'Display post comments', 'foundationpress' ); ?>
			</label>
		</p>
