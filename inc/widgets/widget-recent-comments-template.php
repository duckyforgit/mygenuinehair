...
class Recent_Comments_Widget extends WP_Widget {
	...
	function __construct() {
		...
		parent::__construct(
			'recent-comments', // ID.
			esc_html__( 'Recent Comments (FoundationPress)', 'foundationpress' ), // Name.
			array(
				'classname' => 'recent-comments',
				'description' => esc_html__( 'Displays latest comments with Gravatar.', 'foundationpress' ),
	...
	function widget( $args, $instance ) {
		...
		if ( ! $this->is_preview() ) {
			$cache = wp_cache_get( 'recent_comments', 'widget' );
		}
		if ( ! is_array( $cache ) ) {
			$cache = array();
		}
		...
		if ( $comments ) :
			...
			foreach ( (array) $comments as $comment ) : ?>
				...
				if ( $settings['comment_length'] > 0 ) :  ?>
					...
					<div class="comment-content"><?php echo $this->comment_length( $comment->comment_content, $settings['comment_length'] ); ?></div>
				...
				?>
					...
					<div class="comment-date"><?php echo date( $date_format . ' ' . $time_format , strtotime( $comment->comment_date ) ); ?></div>
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'number' ); ?>"><?php esc_html_e( 'Number of comments to show:', 'foundationpress' ); ?>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'comment_length' ); ?>">
				<?php esc_html_e( 'Excerpt length in number of characters:', 'foundationpress' ); ?>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'avatar' ); ?>">
				...
				<?php esc_html_e( 'Display avatar of comment author', 'foundationpress' ); ?>
			</label>
		</p>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'post_title' ); ?>">
				...
				<?php esc_html_e( 'Display post title of commented post', 'foundationpress' ); ?>
			</label>
		</p>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'comment_date' ); ?>">
				...
				<?php esc_html_e( 'Display comment date', 'foundationpress' ); ?>
			</label>
		</p>
