...
class Tabbed_Content_Widget extends WP_Widget {
	...
	function __construct() {
		...
		parent::__construct(
			'tabbed-content', // ID.
			esc_html__( 'Tabbed Content (Understrap)', 'understrap' ), // Name.
			array(
				'classname' => 'tabbed-content',
				'description' => esc_html__( 'Displays various content with tabs.', 'understrap' ),
	...
	//	wp_enqueue_script( 'tabbed-content', Understrap_PLUGIN_URL . '/assets/js/tabbed-content.js', array( 'jquery' ), Understrap_VERSION );
...
<?php   }  
 ...
 //  }
  ...
  foreach( $categories as $category ) {
    ...
    echo '<li>' . sprintf( esc_html__( '%s', 'textdomain' ), $category_link ) . '&nbsp;&nbsp;(';
    //echo '<p>' . sprintf( esc_html__( 'Description: %s', 'textdomain' ), $category->description ) . '</p>';
    echo '' . sprintf( esc_html__( '%s', 'textdomain' ), $category->count ) . ')</li>';
} 
				// foreach( $categories as $category ) {
				//     $category_link = sprintf( 
				//         '<a href="%1$s" alt="%2$s" class="%3$s">',
				//         esc_url( get_category_link( $category->term_id ) ),
				//         esc_attr( sprintf( __( 'View all posts in %s', 'Understrap' ), $category->name ) ),
				//         esc_attr( sprintf( __( '%s', 'Understrap' ), 'tabcontent-categories-style' ) )
				//        // esc_html( $category->name )
				//     );
				//     //$category_name =  esc_html( $category->name );
				     
				//     echo '<li>' . sprintf( esc_html__( '%s', 'Understrap' ), $category_link ) ;
		 	// 			echo '<span>' . sprintf( esc_html__( '%s', 'Understrap' ), $category->name ) . '</span>';
				//     echo '<span>&nbsp;(' . sprintf( esc_html__( '%s', 'Understrap' ), $category->count ) . ')</span></li>';
				// } 
		...
		<?php endif; ?>
			        <span class="media-object-section main-section">
			         	<a href="<?php echo esc_url(the_permalink( $post )) ?>" title="<?php echo $post->post_title; ?>">
			          	...
			          	<span class="meta-date meta-date">&nbsp;<?php echo $time; //$post->the_time( get_option( 'date_format' ) ); ?></span> </a>
...
<?php }
		...
		break; 
			...
			default: ?>
				...
				<p class="tabcontent-missing">
					<?php esc_html_e( 'Please select the Tab Content in the Widget Settings.', 'Understrap' ); ?>
				</p>
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php esc_html_e( 'Title:', 'Understrap' ); ?>
		...
		for ( $i = 0; $i <= 3; $i++ ) : ?>
			...
			<p style="border-bottom: 1px solid #eaeaea; padding: 10px; margin: 0;">
				...
				<label style="display: inline-block; min-width: 50px" for="<?php echo $this->get_field_id( 'tab_content-' . $i ); ?>">
					<?php printf( esc_html__( 'Tab %s:', 'Understrap' ), $i + 1 ); ?>
				</label>
				...
				<select id="<?php echo $this->get_field_id( 'tab_content-' . $i ); ?>" name="<?php echo $this->get_field_name( 'tab_content-' . $i ); ?>">
					...
					<option value="1" <?php selected( $settings['tab_content'][ $i ], 1 ); ?>><?php esc_html_e( 'Archives', 'Understrap' ); ?></option>
					<option value="2" <?php selected( $settings['tab_content'][ $i ], 2 ); ?>><?php esc_html_e( 'Categories', 'Understrap' ); ?></option>
					<option value="3" <?php selected( $settings['tab_content'][ $i ], 3 ); ?>><?php esc_html_e( 'Pages', 'Understrap' ); ?></option>
					<option value="4" <?php selected( $settings['tab_content'][ $i ], 4 ); ?>><?php esc_html_e( 'Popular Posts', 'Understrap' ); ?></option>
					<option value="5" <?php selected( $settings['tab_content'][ $i ], 5 ); ?>><?php esc_html_e( 'Recent Comments', 'Understrap' ); ?></option>
					<option value="6" <?php selected( $settings['tab_content'][ $i ], 6 ); ?>><?php esc_html_e( 'Recent Posts', 'Understrap' ); ?></option>
					<option value="7" <?php selected( $settings['tab_content'][ $i ], 7 ); ?>><?php esc_html_e( 'Tag Cloud', 'Understrap' ); ?></option>
					<option value="8" <?php selected( $settings['tab_content'][ $i ], 8 ); ?>><?php esc_html_e( 'Related Posts', 'Understrap' ); ?></option>
				</select>

				<br/>

				<label style="display: inline-block; min-width: 50px" for="<?php echo $this->get_field_id( 'tab_titles-' . $i ); ?>">
					<?php esc_html_e( 'Title:', 'Understrap' ); ?>
				</label>
		...
		<strong><?php esc_html_e( 'Settings for Recent/Popular/Related Posts and Recent Comments', 'Understrap' ); ?></strong>

		<p>
			<label for="<?php echo $this->get_field_id( 'number' ); ?>"><?php esc_html_e( 'Number of entries:', 'Understrap' ); ?>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'thumbnails' ); ?>">
				...
				<?php esc_html_e( 'Show Thumbnails?', 'Understrap' ); ?>
			</label>
		</p>
