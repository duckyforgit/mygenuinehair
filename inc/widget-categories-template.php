 ...
 *
 * Display the categories in a boxed layout.
 *
 * @package Understrap
 */
...
class Categories_Sidebar_Widget extends WP_Widget {
	...
	function __construct() {
		...
		parent::__construct(
			'categories-sidebar',  
			esc_html__( 'Categories Sidebar(understrap)', 'understrap' ),  
			array(
				'classname' => 'categories-sidebar',
				'description' => esc_html__( 'Displays categories with image in sidebar.', 'understrap' ),
	...
	public function delete_widget_cache() {
		...
		wp_cache_delete( 'categories_sidebar', 'widget' );
	...
	function widget( $args, $instance ) {
		...
		if ( ! $this->is_preview() ) {
			$cache = wp_cache_get( 'categories_sidebar', 'widget' );
		}
		if ( ! is_array( $cache ) ) {
			$cache = array();
		}
		...
		if ( ! $this->is_preview() ) {
			...
			wp_cache_set( 'categories_sidebar', $cache, 'widget' );
		} else {
			ob_end_flush();
		}
	}
	...
	function postmeta( $settings ) {
		...
		if ( true == $settings['meta_author'] ) : ?>
			...
			<span class="meta-author meta-author">
				<?php printf('<a href="%1$s" title="%2$s" rel="author">%3$s</a>',
					...
					esc_attr( sprintf( esc_html__( 'View all posts by %s', 'understrap' ), get_the_author() ) ),
					get_the_author()
				);?>
			</span>
		...
		if ( true == $settings['meta_comments'] and comments_open() ) : ?>
			...
			<span class="meta-comments">
				<?php comments_popup_link( esc_html__( 'No comments', 'understrap' ), esc_html__( 'One comment','understrap' ), esc_html__( '% comments','understrap' ) ); ?>
			</span>
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'category' ); ?>"><?php esc_html_e( 'Select Category:', 'understrap' ); ?></label><br/>
			<?php
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'order' ); ?>"><?php esc_html_e( 'Order by:', 'understrap' ); ?></label><br/>
			<select name="<?php echo $this->get_field_name( 'order' ); ?>" id="<?php echo $this->get_field_id( 'order' ); ?>">
				<option <?php selected( $settings['order'], 'date' ); ?> value="date"><?php esc_html_e( 'Post Date', 'understrap' ); ?></option>
				<option <?php selected( $settings['order'], 'comment_count' ); ?> value="comment_count"><?php esc_html_e( 'Comment Count', 'understrap' ); ?></option>
				<option <?php selected( $settings['order'], 'rand' ); ?> value="rand"><?php esc_html_e( 'Random', 'understrap' ); ?></option>
			</select>
		</p>

		<p>
			<label for="<?php echo $this->get_field_id( 'number' ); ?>"><?php esc_html_e( 'Number of posts:', 'understrap' ); ?>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'thumbnails' ); ?>">
				...
				<?php esc_html_e( 'Display post thumbnails', 'understrap' ); ?>
			</label>
		</p>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'meta_date' ); ?>">
				...
				<?php esc_html_e( 'Display post date', 'understrap' ); ?>
			</label>
		</p>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'meta_author' ); ?>">
				...
				<?php esc_html_e( 'Display post author', 'understrap' ); ?>
			</label>
		</p>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'meta_comments' ); ?>">
				...
				<?php esc_html_e( 'Display post comments', 'understrap' ); ?>
			</label>
		</p>
